...
{
    public class MesBdUnitDAL : BaseDAL
    {
      public MES_BD_UNIT Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_UNIT Get(string guid, object ts)
        {
            return MES_BD_UNIT.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.BH,A.NAME,A.UNIT_FH,A.CODE
