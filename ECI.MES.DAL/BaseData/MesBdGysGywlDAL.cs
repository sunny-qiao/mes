...
{
    public class MesBdGysGywlDAL : BaseDAL
    {
      public MES_BD_GYS_GYWL Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_GYS_GYWL Get(string guid, object ts)
        {
            return MES_BD_GYS_GYWL.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.FGUID,A.BH,A.GYS,A.NAME
