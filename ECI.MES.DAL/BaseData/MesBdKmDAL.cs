...
{
    public class MesBdKmDAL : BaseDAL
    {
      public MES_BD_KM Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_KM Get(string guid, object ts)
        {
            return MES_BD_KM.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }
