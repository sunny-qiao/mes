...
{
    public class MesBdScxFpDAL : BaseDAL
    {
      public MES_BD_SCX_FP Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_SCX_FP Get(string guid, object ts)
        {
            return MES_BD_SCX_FP.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.FGUID,A.SCX,A.BH,A.FPM
                           ,A.HLLBL,A.HLLRKCK,A.EJPBL,A.EJPCLGX,A.EJPRKCK
