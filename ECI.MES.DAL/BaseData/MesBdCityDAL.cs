...
{
    public class MesBdCityDAL : BaseDAL
    {
      public MES_BD_CITY Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_CITY Get(string guid, object ts)
        {
            return MES_BD_CITY.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }
