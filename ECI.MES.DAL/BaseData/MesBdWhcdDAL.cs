...
{
    public class MesBdWhcdDAL : BaseDAL
    {
      public MES_BD_WHCD Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_WHCD Get(string guid, object ts)
        {
            return MES_BD_WHCD.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }
