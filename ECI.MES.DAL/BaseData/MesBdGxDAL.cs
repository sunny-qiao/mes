...
{
    public class MesBdGxDAL : BaseDAL
    {
      public MES_BD_GX Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_GX Get(string guid, object ts)
        {
            return MES_BD_GX.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.BH,A.NAME,A.CKCK,A.RKCK
                           ,A.CKCPQZM,A.RKCPQZM,A.BMCLYL,A.MD,A.YLSH
