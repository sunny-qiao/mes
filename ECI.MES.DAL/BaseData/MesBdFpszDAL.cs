...
{
    public class MesBdFpszDAL : BaseDAL
    {
      public MES_BD_FPSZ Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_FPSZ Get(string guid, object ts)
        {
            return MES_BD_FPSZ.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.BH,A.NAME,A.HLLBL,A.EJPBL
