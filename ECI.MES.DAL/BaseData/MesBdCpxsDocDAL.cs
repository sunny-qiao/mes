...
{
    public class MesBdCpxsDocDAL : BaseDAL
    {
      public MES_BD_CPXS_DOC Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_CPXS_DOC Get(string guid, object ts)
        {
            return MES_BD_CPXS_DOC.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.FGUID,A.BH,A.NAME,A.FILE_TYPE
