...
{
    public class MesBdZqDAL : BaseDAL
    {
      public MES_BD_ZQ Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_ZQ Get(string guid, object ts)
        {
            return MES_BD_ZQ.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.BH,A.ZQM,A.JZYF,A.QSRQ
                           ,A.JZRQ,A.SJTS,A.IS_JZ,A.JZR,A.SHR
