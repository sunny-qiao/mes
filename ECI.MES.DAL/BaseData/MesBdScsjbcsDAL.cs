...
{
    public class MesBdScsjbcsDAL : BaseDAL
    {
      public MES_BD_SCSJBCS Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_SCSJBCS Get(string guid, object ts)
        {
            return MES_BD_SCSJBCS.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.BH,A.NAME,A.JSFS,A.REMARK
