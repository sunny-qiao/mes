...
{
    public class MesBdDbZdbDAL : BaseDAL
    {
      public MES_BD_DB_ZDB Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_DB_ZDB Get(string guid, object ts)
        {
            return MES_BD_DB_ZDB.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.FGUID,A.BH,A.DBM_MAIN,A.DBM_SUB
