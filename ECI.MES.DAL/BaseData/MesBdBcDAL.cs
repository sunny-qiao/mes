...
{
    public class MesBdBcDAL : BaseDAL
    {
      public MES_BD_BC Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_BC Get(string guid, object ts)
        {
            return MES_BD_BC.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.BH,A.CODE,A.NAME,A.REMARK
