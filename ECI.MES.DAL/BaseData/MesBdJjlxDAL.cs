...
{
    public class MesBdJjlxDAL : BaseDAL
    {
      public MES_BD_JJLX Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_JJLX Get(string guid, object ts)
        {
            return MES_BD_JJLX.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }
