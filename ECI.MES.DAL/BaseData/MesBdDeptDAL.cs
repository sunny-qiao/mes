...
{
    public class MesBdDeptDAL : BaseDAL
    {
      public MES_BD_DEPT Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_DEPT Get(string guid, object ts)
        {
            return MES_BD_DEPT.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }
