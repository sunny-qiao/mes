...
{
    public class MesBdMjlxcsDAL : BaseDAL
    {
      public MES_BD_MJLXCS Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_MJLXCS Get(string guid, object ts)
        {
            return MES_BD_MJLXCS.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.MJLX,A.BH,A.CSM,A.REMARK
