...
{
    public class MesBdWlqzplDAL : BaseDAL
    {
      public MES_BD_WLQZPL Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_WLQZPL Get(string guid, object ts)
        {
            return MES_BD_WLQZPL.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }
