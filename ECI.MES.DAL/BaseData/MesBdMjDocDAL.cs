...
{
    public class MesBdMjDocDAL : BaseDAL
    {
      public MES_BD_MJ_DOC Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_MJ_DOC Get(string guid, object ts)
        {
            return MES_BD_MJ_DOC.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }
