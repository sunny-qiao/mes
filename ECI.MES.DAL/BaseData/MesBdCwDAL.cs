...
{
    public class MesBdCwDAL : BaseDAL
    {
      public MES_BD_CW Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_CW Get(string guid, object ts)
        {
            return MES_BD_CW.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.CK,A.CODE,A.NAME,A.ADDRESS
