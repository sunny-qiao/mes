...
{
    public class MesBdSjkbgDAL : BaseDAL
    {
      public MES_BD_SJKBG Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_SJKBG Get(string guid, object ts)
        {
            return MES_BD_SJKBG.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.BH,A.TABLEM,A.DBM,A.REMARK
