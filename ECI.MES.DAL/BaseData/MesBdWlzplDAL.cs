...
{
    public class MesBdWlzplDAL : BaseDAL
    {
      public MES_BD_WLZPL Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_WLZPL Get(string guid, object ts)
        {
            return MES_BD_WLZPL.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }
