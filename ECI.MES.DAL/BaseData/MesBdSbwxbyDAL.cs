...
{
    public class MesBdSbwxbyDAL : BaseDAL
    {
      public MES_BD_SBWXBY Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_SBWXBY Get(string guid, object ts)
        {
            return MES_BD_SBWXBY.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.BH,A.NAME,A.WXBYNR,A.SCX
                           ,A.UNIT,A.WXBYXE,A.TXXE,A.ZDXE,A.REMARK
