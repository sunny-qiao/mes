...
{
    public class MesBdSbDocDAL : BaseDAL
    {
      public MES_BD_SB_DOC Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_SB_DOC Get(string guid, object ts)
        {
            return MES_BD_SB_DOC.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.FGUID,A.NAME,A.BH,A.FILE_TYPE
