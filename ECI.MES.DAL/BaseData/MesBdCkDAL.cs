...
{
    public class MesBdCkDAL : BaseDAL
    {
      public MES_BD_CK Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_CK Get(string guid, object ts)
        {
            return MES_BD_CK.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.BH,A.CODE,A.NAME,A.IS_XNCK
