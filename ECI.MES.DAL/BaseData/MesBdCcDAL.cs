...
{
    public class MesBdCcDAL : BaseDAL
    {
      public MES_BD_CC Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_CC Get(string guid, object ts)
        {
            return MES_BD_CC.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.BGM,A.BH,A.NAME,A.JD
