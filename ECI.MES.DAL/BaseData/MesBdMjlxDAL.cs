...
{
    public class MesBdMjlxDAL : BaseDAL
    {
      public MES_BD_MJLX Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_MJLX Get(string guid, object ts)
        {
            return MES_BD_MJLX.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.BH,A.NAME,A.CODE,A.DYGX
                           ,A.SYJX,A.MJ_LONG,A.MJ_WIDH,A.REMARK,A.CREATE_USER
                           ,A.CREATE_USER_NAME,A.CREATE_DATE,A.UPDATE_USER,A.UPDATE_USER_NAME,A.UPDATE_DATE
                           ,A.COMPANY_CODE,A.COMPANY_NAME ";
           sql +=" FROM MES_BD_MJLX A ";
