...
{
    public class MesBdGxGzszDAL : BaseDAL
    {
      public MES_BD_GX_GZSZ Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_GX_GZSZ Get(string guid, object ts)
        {
            return MES_BD_GX_GZSZ.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.FGUID,A.IS_ADD,A.QTY,A.IS_WZ
