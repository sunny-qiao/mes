...
{
    public class MesBdDbDAL : BaseDAL
    {
      public MES_BD_DB Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_DB Get(string guid, object ts)
        {
            return MES_BD_DB.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }

        public   DataTable Load(BLLContext context, string guid)
        {
           string sql = @"SELECT A.GUID,A.BH,A.NAME,A.WZ,A.BL
                           ,A.STATUS,A.QYRQ,A.TYRQ,A.JLPCBFB,A.CPSJ
