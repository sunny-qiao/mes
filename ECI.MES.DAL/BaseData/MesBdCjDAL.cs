...
{
    public class MesBdCjDAL : BaseDAL
    {
      public MES_BD_CJ Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_CJ Get(string guid, object ts)
        {
            return MES_BD_CJ.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }
