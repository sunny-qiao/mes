...
{
    public class MesBdGzDAL : BaseDAL
    {
      public MES_BD_GZ Get(string guid)
        {
            return Get(guid, null);
        }

        public MES_BD_GZ Get(string guid, object ts)
        {
            return MES_BD_GZ.DAL.Select().Where(a => a.GUID = guid).SingleOrDefault(ts);
        }
